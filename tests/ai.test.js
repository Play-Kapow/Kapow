...
describe('aiDecideDraw', () => {
  ...
  test('R5T29 guard: draws from deck when power card saves less than 2pts and no completion', () => {
    // Discard is P1 (modifiers [-1, +1]). Best improvement = -(-1) = 1 pt.
    // No triad completion possible. Threshold is >= 2, so AI prefers deck.
    // NOTE: original test used [fd(5), 4, 3] but placing P1(fv=1) at the fd
    // position creates run [1, 3, 4] → aiScorePlacement scores it 30+ → 'discard'.
    // Use values that don't form a run/set with fv=1: [fd(5), 10, 11].
    const aiTriads = [
      { ...makeTriad(0, 0, 0), isDiscarded: true },
      { ...makeTriad(0, 0, 0), isDiscarded: true },
      makeTriad(fc(5, false), fc(10), fc(11)),         // T3: [fd, 10, 11] — no completion with P1
      { ...makeTriad(0, 0, 0), isDiscarded: true },
    ];
